...
@login_required
@permission_required("projects.view_reward", raise_exception=True)
...
@login_required
@permission_required("projects.add_reward", raise_exception=True)
...
@login_required
@permission_required("projects.change_reward", raise_exception=True)
...
@login_required
@permission_required("projects.add_sponsor", raise_exception=True)
...
def reward_sponsor(request, slug):
    ...
    if request.POST:
        ...
        return redirect(
            f"{reverse('projects:reward_sponsor_confirm', kwargs={'slug': project.slug})}?amount={amount}"
        )

    rewards = project.rewards.all()
    return render(
        request, "reward_sponsor_form.html", {"project": project, "rewards": rewards}
    )


@permission_required("projects.view_sponsor", raise_exception=True)
def free_sponsor_confirm(request, slug):
    ...
    return render(
        ...
        {"project": project, "sopnsor": sponsor, "amount": amount, "reward": reward},
    )
...
def reward_sponsor_confirm(request, slug):
    ...
    return render(
        ...
        "rewards/reward_sponsor_confirm.html",
        {
            ...
            "sopnsor": sponsor,
            "amount": amount,
            "is_reward_sponsor": is_reward_sponsor,
            "reward": sponsor.reward if is_reward_sponsor else None,
